...
module Svm where		-- module name and file_name starts with caps, it is made to get imported to test_file.hs
	...
	train_svm !sa !(DataSet data_points labels) epsilon max_iter =
		let	n = snd $ bounds labels
			x_initial = listArray (1, n) $ replicate n 1
			list_ones = listArray (1, n) $ replicate n 1
			a = trace "here2" True
			b = trace ("here3 km=") True	-- ++" "++(showaux km)
			km = buildKMatrix data_points sa
			nu = conjugateGradientAlgo km x_initial list_ones list_ones epsilon max_iter a		--Solving for AX=I case
			v = conjugateGradientAlgo km x_initial labels labels epsilon max_iter b			--Solving for AX=Y case
			bias = (aSum v) / (aSum nu)
