...
tdm titles = buildMatrix (length $ tf titles) (length $ removeStopwords titles) ( \(term,doc)->
	let	termDocFreq = ( fromIntegral ( termFreqDoc titles doc $ fst ( tf titles !! term ) ) ) / ( genericLength $ getDoc titles doc)
		idf = (genericLength $ removeStopwords titles) / ( fromIntegral (snd ( tf titles !! term ) ))
	in termDocFreq * log(idf)) 

lsa :: [String]->Int->Matrix Double
lsa titles k = u<>sigma<>v
	where
		(u,sigma,v)=fullSVD $ tdm titles
		uk = takeColumns k u
		vk = takeRows k u
		sigmak = takeColumns k $ takeRows k sigma
--matrix = buildMatrix (length vocabulary) (length removeStopwords) (\(term,doc)->termFrequency (vocabulary !! term) doc)
